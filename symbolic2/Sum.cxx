...
Expression const& Sum::combine(Expression const& arg1, Expression const& arg2)
{
  ...
  return Product::multiply(arg1.get_constant_factor() + arg2.get_constant_factor(), factor1);
}
    ...
    else
    {
      if (is_less(arg1, sum2->arg1_))
      {
        if (Constant::is_zero(arg1))
          ...
          return realize(arg1, *sum2);
      }
      else if (is_less(sum2->arg1_, arg1))
        return realize(sum2->arg1_, add(arg1, sum2->arg2_));
      else
        return add(combine(arg1, sum2->arg1_), sum2->arg2_);
    }
  }
  else if (sum2)
  {
    if (is_less(sum1->arg1_, sum2->arg1_))
      return add(sum1->arg1_, add(sum1->arg2_, *sum2));
    else if (is_less(sum2->arg1_, sum1->arg1_))
      return add(sum2->arg1_, add(*sum1, sum2->arg2_));
    else
      return add(add(combine(sum1->arg1_, sum2->arg1_), sum1->arg2_), sum2->arg2_);
  }
  else
  {
    if (is_less(sum1->arg1_, arg2))
      return realize(sum1->arg1_, add(arg2, sum1->arg2_));
    else if (is_less(arg2, sum1->arg1_))
      return realize(arg2, *sum1);
    else
      return add(combine(sum1->arg1_, arg2), sum1->arg2_);
  }
}
